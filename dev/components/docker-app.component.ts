...
export class DockerAppComponent {
    ...
    getConfigCallback(): void {
        ...
        this.connectionService.connectionObservable.subscribe((wsData) => {
            ...
            this.dockerKeys.forEach((dockerKey) => {
                if (wsData.source === this.dockers[dockerKey].source) selectedDockerKeys.push(dockerKey);
            });

            if (processedIds.length < selectedDockerKeys.length) {

                selectedDockerKeys.forEach((dockerKey) => {
